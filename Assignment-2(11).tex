...
\end{bmatrix}
\end{align}
where I,O are Identity and Zero matrices and R,Q are other sub-matrices respectively.
...
here F=\brak{I-Q}$^{-1}$ is called the fundamental matrix of P.
\end{lemma}
...
\end{bmatrix}
\end{align}
Here an element $P_{ij}$ represents the probability of state j starting from state i. Let A be the event of getting the first head to appear on odd numberth trail starting with a odd numberth trail. Then the probability of A is   
\begin{align}
   \Pr(A) = p_{14} 
   & = 0.667
\end{align}
